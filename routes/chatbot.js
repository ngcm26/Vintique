...
router.post('/send', async (req, res) => {
  ...
  if (/order|where.*order|status.*order|what.*buy|what.*purchase/i.test(message)) {
    try {
      ...
      if (orders.length > 0) {
        ...
        // Fetch items of the latest order including main image if any
        ...
        // Prepare the items details string
        const itemsText = items.map(item =>
          `${item.product_name} (x${item.quantity}) - $${Number(item.price).toFixed(2)}`
        ).join('\n');


        // Use first item's image URL if available
